...
function hAttributes(node, transform) {
    var attributes = node.attributes.slice();
    ...
    var useCompactPropertiesDefinition = attributes.every(function(attr) {
        return (
    ...
    var propertyAttributes = attributes.filter(isPropertyAttribute);

    var ordinaryAttributes = attributes
...
module.exports = function(node, transform) {
    ...
    var hasContent = (
        ...
        content.length > 0 &&
        // Content from `<textarea>` tag has to be moved to a value attribute:
        //
        //     <textarea><TMPL_VAR content></textarea>
        //
        // to
        //
        //     <textarea value="<TMPL_VAR content>"></textarea>
        //
        // Otherwise the textarea value will not be updated.
        !isTextarea(node)
    );
