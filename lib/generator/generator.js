        ...
        if (this.node.name === 'TMPL_ASSIGN') {
            ...
            return this.update({
                ...
                "arguments": [
                    ...
                    transform(this.node.attributes[1].content)
                ]
            }, true);
        }
...
function blockDeclaration(attributes, content) {
    var blockName = getPrimaryAttributeValue(attributes);

    return {
        type: 'FunctionDeclaration',
        id: {
            type: 'Identifier',
            name: 'block_' + blockName
        },
        params: [],
        defaults: [],
        body: {
...
function returnLastStatement(body) {
    return body.map(function(statement, index) {
