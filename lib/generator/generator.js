...
function generator(string) {
    ...
    function buildTmplCall(value) {
        var match = value.match(/([a-zA-Z]+)\((.*)\)/);

        var name = match[1];

        var z = traverse(parser.parse(match[2])).map(handler);
    ...
    return templateRuntime.toString().replace('// return body', 'return ' + body);
}
